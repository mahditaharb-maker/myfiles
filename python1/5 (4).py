...
def compute_Tp(p):
    ...
    where v = (p-3)//2 (number of terms v+1).
    """
    v = (p - 3) // 2
    s = Fraction(0, 1)
    for k in range(v + 1):
...
def fractional_part(frac):
    ...
    # int(frac) works with Fraction -> truncates toward zero; safe here
...
def format_row(p, n, Qmin):
    ...
    pi_Q = float(4 * dyadic_frac)
    binary_err = f"2^-{n}"
    bit_str = "(" + ",".join(str(b) for b in bits) + ")"
    frac_str = f"{dyadic_frac.numerator}/{dyadic_frac.denominator}"
    print(f"{p:<3} {n:<3} {Qmin:<6} {bit_str:<40} {frac_str:<22} {dyadic_dec:<18.15f} {pi_Q:<18.12f} {binary_err}")

def main():
    # Header
    print(f"{'p':<3} {'n':<3} {'Qmin':<6} {'dyadic bits':<40} {'dyadic (frac)':<22} {'dyadic (dec)':<18} {'π_Q (approx)':<18} {'binary err'}")
    print("-" * 130)
