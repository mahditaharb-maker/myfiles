...
def find_irred_poly(p, k):
    ...
    for coeffs in product(range(p), repeat=k):
        poly = Poly(x**k + sum(c*x**i for i,c in enumerate(coeffs)), x, domain=GF(p))
...
def build_field(p, k, irr_poly=None):
    ...
    def to_vec(poly):
        remp = rem(Poly(poly, x, domain=GF(p)), irr_poly, domain=GF(p))
        coeffs = [0]*k
...
def tex_table(elems, op, fmt):
    ...
    lines = []
    # header
    lines.append("\\begin{array}{c|" + "c"*n + "}")
...
def generate_gf_tex(n):
    ...
    if p is None:
        print(f"Error: {n} is not a prime power.")
        sys.exit(1)
...
\[
\\text{{Multiplication in }}\\mathbb{{F}}_{{{n}}}=
{mul_tbl}
\\]
...
if __name__ == "__main__":
    # usage: python 2.py [n], default n=9
